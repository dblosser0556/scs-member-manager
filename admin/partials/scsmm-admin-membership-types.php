...
<div class="wrap">
  <h1 class="wp-heading-inline"><?= __('Manage Membership Types', 'scsmm');?></h1>
  <a class="page-title-action" href="<?= admin_url("admin.php?page=scsmm-membership-type") ?>"><?= __('Create', 'scsmm');?></a>
  ...
  <table class="wp-list-table widefat fixed striped posts">
    ...
    <tbody>
      <?php for($i=0;$i<sizeof($membershiptypes);$i++) { $item = $membershiptypes[$i]; ?>
        <tr>
          <td><?= $item->name ?></td>
          <td><a class="page-action" href="<?= admin_url("admin.php?page=scsmm-membership-type&typeID={$item->id}") ?>"><?= __('Edit', 'scsmm');?></a>
        </tr>
      <?php } ?>
    </tbody>
  </table>
